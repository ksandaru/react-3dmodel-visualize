...
export default function Modelsmallcopy({props}) {
    ...
    const filtered = data.filter((uhf) => uhf.id === 5)
    const mapped = filtered.map((statuss)=> statuss.status)
    console.log(mapped)

    const colorChange=()=>{
        if(mapped.toString() === 'TIGHT'){
            setColor(snap.items.location1)
        } else setColor(snap.items.location2)
    }
        ...
        <>
            <group ref={myGroup} {...props} dispose={null}>
                ...
                <group position={[176.89, 34.45, 172.83]} rotation={[-Math.PI / 2, 0, 0]} scale={20}>
                    <group rotation={[Math.PI / 2, 0, 0]}>
                        <mesh geometry={nodes.Object_4001.geometry} material={materials['material.004']}
                              material-color={snap.items.location2}/>
                ...
                <group position={[176.87, 34.45, -172.14]} rotation={[-Math.PI / 2, 0, 0]} scale={20}>
                    <group rotation={[Math.PI / 2, 0, 0]}>
                        <mesh idd= {5} geometry={nodes.Object_4014.geometry} material={materials['material.017']}
                              material-color={
                                  mapped.toString() ==='LOOSE' ? snap.items.location1: snap.items.location2
                                  }
                        />
